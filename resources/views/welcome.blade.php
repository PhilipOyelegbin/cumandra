        ...
        <section class="space-y-6 md:space-y-10 flex flex-col md:flex-row gap-3 px-6 md:px-10">
            <article class="space-y-3 md:max-w-4/6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <h2 class="text-3xl font-bold">{{ $articles['sports']['articles'][0]['headline'] }}</h2>
                    <div class="space-y-3">
                        <p class="text-sm text-darkGray dark:text-grayBlue">{{ $articles['sports']['articles'][0]['description'] }}</p>
                        <a href="{{ $articles['sports']['articles'][0]['url'] }}" class="inline-block bg-softRed text-secondary hover:bg-secondary hover:text-primary ease-linear duration-300 px-4 py-2" target="_blank" rel="noopener noreferrer">Read more</a>
                    </div>
                </div>
            </article>
